...
function tranS(rat,kaat,ksat)
    return (normfreeA(rat,ksat)) ./ (kaat+normfreeA(rat,ksat))
end
...
parameterfull = (a1=norma1, b1=normb1, stoich=[[1, 0, 0], [-1, 0, 0], [0, -1, 1], [0, 1, -1]])
# parameter values and stoichometry matrix

num=1000
seqfull(u, p, t) = [p.a1*u[3], p.b1*u[1], normfreeA(rat,ksat)*u[2], kaat*u[3]]
s0 = [0, 1, 0] # M, EF, EA
global saveM = zeros(Float64,num,numpt+1)
global saveEF = zeros(Float64,num,numpt+1)
global saveEA = zeros(Float64,num,numpt+1)
for iter in 1:num
    solseq2 = ssa_direct(seqfull, s0, tend, parameterfull, parameterfull.stoich)
    saveM[iter,:] = (solseq2.u[1,:])
    saveEF[iter,:] = (solseq2.u[2,:])
    saveEA[iter,:] = (solseq2.u[3,:])
    if iter%100==0
        println(iter)
    end
end
